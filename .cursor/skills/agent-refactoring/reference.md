...
Читай вместе со SKILL.md при рефакторинге `application/agents/` под черновик
«СОЗДАНИЕ УНИВЕРСАЛЬНОЙ RAG-СИСТЕМЫ С АГЕНТНОЙ ПАМЯТЬЮ...». Этот файл —
карта соответствия: что из черновика уже принято правилами 000–099, что
заменено более поздним решением, и что требует явного ответа пользователя
перед реализацией (090/000: «спроси, не выдумывай контракт»).

При противоречии этого файла и правил (000–099) — правила побеждают (055).
Черновик как единый .md-файл в `.cursor` не заводим — он вводит параллельный
источник истины по осям/реестру/стеку, что запрещено 050 (дублирование =
антипаттерн).
...
| Learning Agent | **отсутствует в реестре** | Не добавлять молча — см. §5 (открытый вопрос) |
...
| xMemory (decouple-before-aggregate) | Не зафиксировано в 060. Потенциальное улучшение consolidation — см. §5 | 060 |
| Learning Agent + периодический fine-tuning | Не в реестре, не в стеке — см. §5 | 000 |

## 5. Открытые вопросы — спроси пользователя, не решай молча

Правило 090/000: если контракта не хватает — спрашивать, а не изобретать.
Перед реализацией уточни:

1. **Learning Agent** — добавлять в реестр 000 как новую роль (петля
   обратной связи, дообучение ранжировщика) или явно исключить из scope
   MAX PRO LEVEL? Роль не следует создавать «по аналогии» без правки 000.
2. **xMemory-стиль consolidation** (decouple компонентов памяти перед
   агрегацией) — принять как реализацию шага consolidation в 060, или
   оставить текущую схему (`access_frequency >= N` + `importance >= M` →
   узел графа)?
3. **RAGAS-критерии** — включить Faithfulness/Context Relevancy в промпт
   LLM-as-a-Judge (040) как дополнительные пункты оценки, или оставить
   текущие accuracy/safety?
4. **CAG/prompt-кэширование** — заводить как политику `RouteStrategy`
   (055) с явным именем стратегии (например, `cached_response`) в закрытом
   перечне, или не реализовывать в этой волне?

Пока ответа нет — эти пункты не реализуются «по умолчанию» ни в одной волне
